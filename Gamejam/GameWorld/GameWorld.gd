...
func _physics_process(delta):
	if last_platform_componets!=[]:
		var distance=player.global_position.distance_to(last_platform_componets[0].global_position)
		print(distance)
		if distance<1500:
			spawn_platforms()
...
func spawn_platforms():
	last_platform_componets=spawn_platform(ending_position)
	ending_position=(last_platform_componets[0].end_point.global_position)
	print(ending_position)
func spawn_platform(Position)->Array:
